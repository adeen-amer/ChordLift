...
Excluding the guard track, the fine-tune is close to a wash on DEV: small mixed deltas in both directions (−0.04 to +0.05), no systemic regression. The failure is concentrated entirely on `another-one-bites-the-dust`, whose root/majmin/sevenths all collapsed near zero (`ref=105 est=47` segments) while `seg=0.567` stayed moderate — consistent with the decoder finding few confident segments rather than a wholesale extraction failure. This is the exact track v50's `CHORD_PITCH_MAX_CANDIDATE_SHIFT=0.40` cap exists to protect against a known 0.45st tuning-estimator-error candidate; the fine-tuned checkpoints' confidence calibration likely shifted enough that the raw-vs-corrected branch selection (`CHORD_PITCH_SELECT=confidence`) picks the wrong branch for this specific track, even though the cap value itself was left unchanged.

**Not investigated further this session:** whether the guard-track regression is fixable by re-tuning the pitch-selection cap/margin against the fine-tuned checkpoints specifically, or whether it's a deeper calibration issue with fine-tuning under this recipe. That's the natural next step if Phase B is revisited.
